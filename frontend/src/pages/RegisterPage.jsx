...
import { BookOpen, AlertCircle, ArrowLeft, Key, Mail, User } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleGoogleCredentialResponse = async (response) => {
    ...
    if (result.success) {
      ...
      if (result.error.non_field_errors) {
        ...
        if (mappedErrors.password1) {
          mappedErrors.password = mappedErrors.password1;
        }
  ...
  return (
    <div className="min-h-screen bg-cream flex flex-col justify-center items-center px-4 relative overflow-hidden">
      ...
      <div className="absolute w-64 h-64 -top-32 -left-32 bg-rejuvenate/20 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute w-64 h-64 -bottom-32 -right-32 bg-rejuvenate/20 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="w-full max-w-md bg-cream border-2 border-specialOps/40 p-8 rounded-3xl shadow-cozy relative">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="font-serif text-3xl font-bold text-nettle">Create Account</h2>
          <p className="text-sm text-specialOps font-medium mt-1">Join our cozy circle of readers</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-sm font-semibold text-nettle mb-1.5" htmlFor="displayName">
              Display Name
            </label>
            <div className="relative">
              <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-specialOps" />
              <input
                id="displayName"
                type="text"
                placeholder="Jane Bennet"
                value={displayName}
                onChange={(e) => {
            ...
            {errors.displayName && (
              <p className="text-red-700 text-xs mt-1 font-semibold">{errors.displayName}</p>
            )}
          </div>
          ...
          <div>
            <label className="block text-sm font-semibold text-nettle mb-1.5" htmlFor="password">
              Password
            </label>
            <div className="relative">
        ...
        <p className="text-center text-sm text-pepper/80 mt-8">
          ...
          <Link to="/login" className="font-bold text-nettle hover:underline">
            Sign In Instead
          </Link>
        </p>
      </div>
